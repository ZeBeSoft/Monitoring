...
class Fronius_Symo_GEN24_Plus:
    name = 'Fronius_Symo_GEN24_Plus'
    ...
    def load_data_fromurl(self):
        logger.info('load_data_fromurl: ->')
        data = OrderedDict()
        base_url = "http://" + self.configuration[self.name]['IPAdresse'] + "/solar_api/GetAPIVersion.cgi"
        self.timestamp = datetime.now(self.tz)
        if self.http is None:
            self.http = urllib3.PoolManager()
        json_url = self.http.request('GET', base_url, retries=False)
        url_response = json.loads(json_url.data.decode('utf-8'))
		#
		# http://fronius-keller/solar_api/GetAPIVersion.cgi
		# http://fronius-keller/solar_api/v1/GetInverterInfo.cgi
		# http://fronius-keller/solar_api/v1/GetActiveDeviceInfo.cgi?DeviceClass=System
		# http://fronius-keller/solar_api/v1/GetInverterRealtimeData.cgi?Scope=System
		# http://fronius-keller/solar_api/v1/GetInverterRealtimeData.cgi?Scope=Device&DataCollection=CumulationInverterData
		# http://fronius-keller/solar_api/v1/GetInverterRealtimeData.cgi?Scope=Device&DataCollection=CommonInverterData
		# http://fronius-keller/solar_api/v1/GetInverterRealtimeData.cgi?Scope=Device&DataCollection=3PInverterData
		# http://fronius-keller/solar_api/v1/GetPowerFlowRealtimeData.fcgi
		# http://fronius-keller/solar_api/v1/GetMeterRealtimeData.cgi?Scope=System
		# 
        #
        ...
        # logger.info('url_response: %s' % url_response)
        logger.info('APIVersion: %s' % url_response['APIVersion'])
        logger.info('BaseURL: %s' % url_response['BaseURL'])
        # logger.info('url_response: %s' % url_response)
        if check_values_empty(data):
            logger.error('empty strings were found')
            raise ValueError('empty strings were found')
